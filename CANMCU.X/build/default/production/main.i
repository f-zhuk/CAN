...
# 1 "main.c" 2
# 44 "main.c"
# 1 "./CANopen/CO_MAIN.h" 1
# 45 "./CANopen/CO_MAIN.h"
# 1 "./CANopen/CO_DEFS.def" 1
# 45 "./CANopen/CO_MAIN.h" 2
...
# 56 "./CANopen/CO_MAIN.h" 2
# 80 "./CANopen/CO_MAIN.h"
...
unsigned char uIOinFilter;
unsigned char uIOinPolarity;
unsigned char uIOinIntChange;
unsigned char uIOinIntRise;
unsigned char uIOinIntFall;
unsigned char uIOinIntEnable;
...
unsigned char uLocalXmtBuffer[8];
unsigned char uLocalRcvBuffer[8];

UNSIGNED8 uDemoState;
unsigned char uDemoSyncCount;
unsigned char uDemoSyncSet;
# 100 "main.c"
void DemoInit(void)
{
 uDemoSyncSet = 255;
 ...
 _uCOB_ID_in.word = _uCO_nodeID.byte + 0xC0000180L; _CO_COB_CANopen2MCHP();;


 uTPDOComm1 .word = _uCOB_ID_out.word;


 _uCOB_ID_in.word = _uCO_nodeID.byte + 0xC0000200L; _CO_COB_CANopen2MCHP();;


 uRPDOComm1 .word = _uCOB_ID_out.word;
...
void CO_COMM_RPDO1_COBIDAccessEvent(void)
{
 switch (uDict.cmd)
 {
  ...
  case DICT_OBJ_READ:
            ...
            _uCOB_ID_in.word = uRPDOComm1.word; _CO_COB_MCHP2CANopen();;
  ...
  case DICT_OBJ_WRITE:
   ...
   _uCOB_ID_in.word = *(unsigned long *)(uDict.obj->pReqBuf); _CO_COB_CANopen2MCHP();;


   if ((*(UNSIGNED32 *)(&_uCOB_ID_out.word)).bytes.B1.bits.b4)
   {


                if (!((_uCOB_ID_out.word ^ uRPDOComm1.word) & 0xFFFFEFFFL))
    {

     if (_rpdo_en.bits.b0) {_CO_COMM_PDO1_Close();}



                    (*(UNSIGNED32 *)(&uRPDOComm1.word)).bytes.B1.bits.b4 = 1;
    }
    else {uDict.ret = E_PARAM_RANGE;}
   }
   ...
   else
   {
    ...
    if (!((_uCOB_ID_out.word ^ uRPDOComm1 .word) & 0xFFFFEFFFL))
    {
     ...
     if (!_rpdo_en.bits.b0) {_CO_COMM_PDO1_Open();}


     (*(UNSIGNED32 *)(&uRPDOComm1 .word)).bytes.B1.bits.b4 = 0;
    }
    else {uDict.ret = E_PARAM_RANGE;}
   }
   break;
...
void CO_COMM_TPDO1_COBIDAccessEvent(void)
{
 switch (uDict.cmd)
 {
  case DICT_OBJ_INFO:
   ...
   _uCOB_ID_in.word = uTPDOComm1 .word; _CO_COB_MCHP2CANopen();;
  ...
  case DICT_OBJ_WRITE:
   ...
   if ((*(UNSIGNED32 *)(&_uCOB_ID_out.word)).bytes.B1.bits.b4)
   {
    ...
    if (!((_uCOB_ID_out.word ^ uTPDOComm1 .word) & 0xFFFFEFFFL))
    {

     if (_tpdo_en.bits.b0) {_tpdo_en.bits.b0 = 0;}


     (*(UNSIGNED32 *)(&uTPDOComm1 .word)).bytes.B1.bits.b4 = 1;
    }
    else {uDict.ret = E_PARAM_RANGE;}
   }
   ...
   else
   {
    ...
    if (!((_uCOB_ID_out.word ^ uTPDOComm1 .word) & 0xFFFFEFFFL))
    {
     ...
     if (!_tpdo_en.bits.b0) {_tpdo_en.bits.b0 = 1;}


     (*(UNSIGNED32 *)(&uTPDOComm1 .word)).bytes.B1.bits.b4 = 0;
    }
    else {uDict.ret = E_PARAM_RANGE;}
   }
   break;
