...
# 2 "<built-in>" 2
# 1 "CANopen/CO_COMM.c" 2
# 46 "CANopen/CO_COMM.c"
# 1 "CANopen/CO_DEFS.def" 1
# 46 "CANopen/CO_COMM.c" 2
...
# 54 "CANopen/CO_COMM.c" 2



# 1 "CANopen/CO_PDO.h" 1
# 50 "CANopen/CO_PDO.h"
...
void _CO_COMMResetEventManager(void)
{
 ...
 _uNMTEState.bits.b0 = 1;
 _uCAN_Handle = 0; _CANIsPutReady();;
 _CO_COMM_NMTE_TXEvent();
 RXB0CONbits.FILHIT3 = 1;


 _uCO_state.bits.b0 = 1;
}
# 186 "CANopen/CO_COMM.c"
void _CO_COMMRXEventManager(void)
{
 ...
 if (_uCO_state.bits.b0)
 if (_uCAN_ret)
 {

  switch(_uCAN_ret & 0xC0)
  {
   case 0x00:
    switch(_uCAN_ret & 0x3F)
    {
     case 1:
      if (_net_en.bits.b0) _CO_COMM_NMT_RXEvent();
      break;
     case 2:
      if (_net_en.bits.b1) {if (!_uCO_state.bits.b2) _CO_COMM_SYNC_RXEvent();}
      break;



     case 4:
      if (_net_en.bits.b3) _CO_COMM_NMTE_RXEvent();
      break;
     default:
   ...
   case 0x40:
    if (!_uCO_state.bits.b2)
    {
     switch(_uCAN_ret & 0x3F)
     {
      case 0:
       if (_sdo_en.bits.b0) _CO_COMM_SDO1_RXEvent();
       break;
# 240 "CANopen/CO_COMM.c"
      default:
   ...
   case 0x80:
    if (!_uCO_state.bits.b2)
    if (_uCO_state.bits.b1)
    {
     switch(_uCAN_ret & 0x3F)
     {
      case 0:
       if (_rpdo_en.bits.b0) {if (!_rx_pdo_flg.bits.b0){_rx_pdo_flg.bits.b0 = 1; _CO_COMM_PDO1_RXEvent();}}
       break;
# 285 "CANopen/CO_COMM.c"
      default:
  ...
  RXB0CONbits.RXFUL = 0; PIR5bits.RXB1IF = 0;;
 }
}
# 322 "CANopen/CO_COMM.c"
void _CO_COMMTXRdyEventManager(void)
{


  _uCAN_Handle = 1; _CANIsPutReady();
 ...
 if (_uCAN_ret)
 {
  if (_tx_net_que.byte)
  {
   if (_tx_net_que.bits.b3)
   {
    if (_net_en.bits.b3) {_uCAN_Handle = 0x00 | 4; _CANIsPutReady();; _CO_COMM_NMTE_TXEvent(); RXB0CONbits.FILHIT3 = 1;}
    _tx_net_que.bits.b3 = 0;
   }
   else
# 352 "CANopen/CO_COMM.c"
   {_tx_net_que.byte = 0;}
  }
  else
  ...
  if (_tx_sdo_que.byte)
  {
   if (!_uCO_state.bits.b2)
   {
    if (_tx_sdo_que.bits.b0)
    {
     if (_sdo_en.bits.b0) {_uCAN_Handle = 0x40 | 0; _CANIsPutReady();; _CO_COMM_SDO1_TXEvent(); RXB0CONbits.FILHIT3 = 1;}
     _tx_sdo_que.bits.b0 = 0;
    }
    else
# 386 "CANopen/CO_COMM.c"
    {_tx_sdo_que.byte = 0;}
   }
  }
  else
  ...
  if (_tx_pdo_que.byte)
  {
   ...
   if (!_uCO_state.bits.b2)
   {
    ...
    {
     if (_tpdo_en.bits.b0) {_uCAN_Handle = 0x80 | 0; _CANIsPutReady();; _CO_COMM_PDO1_TXEvent(); RXB0CONbits.FILHIT3 = 1;}
     _tx_pdo_que.bits.b0 = 0;
    }
    else
# 451 "CANopen/CO_COMM.c"
    {_tx_pdo_que.byte = 0;}
   }
  }
 }
}
# 474 "CANopen/CO_COMM.c"
void _CO_COMMTXFinEventManager(void)
{
...
# 520 "CANopen/CO_COMM.c"
   ...
   case 0x80:
    ...
    if (!_uCO_state.bits.b2)
    {
     ...
     {
      case 0:
       if (_tpdo_en.bits.b0){CO_PDO1TXFinEvent();}
       break;
# 564 "CANopen/CO_COMM.c"
      default:
...
void _CO_COMMLSTimeEventManager(void)
{
 ...
 if (_uCO_state.bits.b0)
 {
  ...
  if (_tpdo_en.byte)
  {
   if (_tpdo_en.bits.b0){CO_PDO1LSTimerEvent();}
# 620 "CANopen/CO_COMM.c"
  }
  ...
  if (_net_en.byte)
  {
   ...
   if (_net_en.bits.b3){_CO_COMM_NMTE_LSTimerEvent();}
  ...
  if (_sdo_en.byte)
  {
   if (_sdo_en.bits.b0){_CO_COMM_SDO1_LSTimerEvent();}
